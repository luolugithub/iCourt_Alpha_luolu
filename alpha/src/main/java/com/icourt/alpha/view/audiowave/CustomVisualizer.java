package com.icourt.alpha.view.audiowave;
...
/**
 * Created by zhaolu on 17/05/26.
 */
public class CustomVisualizer extends LinearLayout implements IVisualizer {
    ...
    public CustomVisualizer(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void attributes(AttributeSet attrs) {
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.audiowaves_style,
                0, 0);

        try {
            FORMAT = a.getInteger(R.styleable.audiowaves_style_aw_format, FORMAT);
            VISUALIZER_HEIGHT = a.getInteger(R.styleable.audiowaves_style_aw_height, VISUALIZER_HEIGHT);
            VISUALIZER_WIDTH = a.getInteger(R.styleable.audiowaves_style_aw_width, VISUALIZER_WIDTH);
            VISUALIZER_NUM_WAVES = a.getInteger(R.styleable.audiowaves_style_aw_num_waves, VISUALIZER_NUM_WAVES);
            VISUALIZER_GRAVITY = a.getInteger(R.styleable.audiowaves_style_aw_gravity, VISUALIZER_GRAVITY);

            LINE_WIDTH = a.getInteger(R.styleable.audiowaves_style_aw_line_with, LINE_WIDTH);
            LINE_MIN_WIDTH = a.getInteger(R.styleable.audiowaves_style_aw_line_min_with, LINE_MIN_WIDTH);
            LINE_HEIGHT = a.getInteger(R.styleable.audiowaves_style_aw_line_height, LINE_HEIGHT);
            LINE_MIN_HEIGHT = a.getInteger(R.styleable.audiowaves_style_aw_line_min_height, LINE_MIN_HEIGHT);
            LINE_SPACING = a.getInteger(R.styleable.audiowaves_style_aw_line_spacing, LINE_SPACING);
            LINE_BORDER_RADIUS = a.getInteger(R.styleable.audiowaves_style_aw_line_border_radius, LINE_BORDER_RADIUS);
            BALL_DIAMETER = a.getInteger(R.styleable.audiowaves_style_aw_ball_diameter, BALL_DIAMETER);

            COLOR_UNIFORM = a.getColor(R.styleable.audiowaves_style_aw_color_uniform, getResources().getColor(COLOR_UNIFORM));
            COLOR_IS_GRADIENT = a.getBoolean(R.styleable.audiowaves_style_aw_color_is_gradient, COLOR_IS_GRADIENT);
            COLOR_GRADIENT_START = a.getColor(R.styleable.audiowaves_style_aw_color_gradient_start, getResources().getColor(COLOR_GRADIENT_START));
            COLOR_GRADIENT_END = a.getColor(R.styleable.audiowaves_style_aw_color_gradient_end, getResources().getColor(COLOR_GRADIENT_END));
        } finally {
            a.recycle();
        }
    }
    ...
    public void prepare(int pitchInHz) {
        ...
        for (int i = 0; i < waveList.size(); i++) {
            ...
            switch (FORMAT) {
                ...
                case 2:
                    params = new LayoutParams(
                            LINE_WIDTH,
                            size < LINE_MIN_HEIGHT ? LINE_MIN_HEIGHT : size < 100 ? 100 : size);
                    params.setMargins(LINE_SPACING, 0, LINE_SPACING, 0);
                    break;
                case 3:
                    params = new LayoutParams(
