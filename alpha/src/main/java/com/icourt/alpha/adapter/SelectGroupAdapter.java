...
public class SelectGroupAdapter extends MultiSelectRecyclerAdapter<GroupBean> {
    @Override
    ...
    @Override
    public void onBindSelectableHolder(ViewHolder holder, GroupBean groupBean, boolean selected, int position) {
        TextView nameView = holder.obtainView(R.id.group_name_tv);
        ImageView arrowView = holder.obtainView(R.id.group_isselect_view);

        nameView.setText(groupBean.getName());
