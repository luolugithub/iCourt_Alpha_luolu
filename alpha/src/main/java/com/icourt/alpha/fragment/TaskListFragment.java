    ...
    public void onDeleteTaskEvent(TaskActionEvent event) {
        ...
        switch (event.action) {
            ...
            case TaskActionEvent.TASK_DELETE_ACTION:
                ...
                if (type == TYPE_ALL) {
                    //未完成
                    if (stateType == Const.TASK_STATETYPE_UN_FINISH) {
                        //删除动作暂时重新请求接口
                        getData(true);
                    } //已完成
                    else if (stateType == Const.TASK_STATETYPE_FINISH) {
                        if (taskAdapter != null) {
                            taskAdapter.removeItem(event.entity);
                            enableEmptyView(taskAdapter.getData());
                        }
                    } //已删除
                    else if (stateType == Const.TASK_STATETYPE_DELETED) {
                        if (taskAdapter != null) {
                            //从已删除列表中彻底删除
            ...
            case TaskActionEvent.TASK_ADD_ITEM_ACITON:
                ...
                if (type == TYPE_ALL) {
                    ...
                    if (stateType == Const.TASK_STATETYPE_FINISH || stateType == Const.TASK_STATETYPE_DELETED) {
                        if (taskAdapter != null) {
                            taskAdapter.addData(event.entity);
                        }
                    } //未完成的，暂时走刷新逻辑
...
//                        onTasksChangeListener.onTasksChanged(response.body().result.items);
    ...
    private void getTaskGroupData(final TaskEntity taskEntity) {
        ...
        else if (stateType == Const.TASK_STATETYPE_FINISH || stateType == Const.TASK_STATETYPE_DELETED) {
    ...
    public void onItemChildClick(BaseQuickAdapter baseQuickAdapter, View view, int i) {
        ...
        switch (view.getId()) {
            ...
            case R.id.task_item_checkbox:
                ...
                //已完成／未完成列表
                if (stateType == Const.TASK_STATETYPE_UN_FINISH || stateType == Const.TASK_STATETYPE_FINISH) {
                    //完成任务
                    if (!itemEntity.state) {
