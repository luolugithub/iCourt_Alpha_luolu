 ...
 * version 2.1.0
 */
public class FolderDetailDialogFragment extends BaseDialogFragment {
    ...
    public static void show(@NonNull String folderId,
                            @NonNull FragmentManager fragmentManager) {
        if (fragmentManager == null) return;
        String tag = FolderDetailDialogFragment.class.getSimpleName();
        FragmentTransaction mFragTransaction = fragmentManager.beginTransaction();
        Fragment fragment = fragmentManager.findFragmentByTag(tag);
        if (fragment != null) {
            mFragTransaction.remove(fragment);
        }
        show(newInstance(folderId), tag, mFragTransaction);
    }

    public static FolderDetailDialogFragment newInstance(@NonNull String folderId) {
    ...
    protected void initView() {
        ...
        baseFragmentAdapter.bindTitle(true,
                Arrays.asList("修改历史", "内部共享", "下载链接", "上传链接"));
