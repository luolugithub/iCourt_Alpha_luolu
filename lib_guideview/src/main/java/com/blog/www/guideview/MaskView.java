...
class MaskView extends ViewGroup {
    ...
    private OnTargetViewClick onTargetViewClick;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isTouchTargetView(downX, downY) && isTouchTargetView(upX, upY)) {
                    //说明点击了高亮区域
                    if (onTargetViewClick != null) {
                        onTargetViewClick.onClick(this);
                        Toast.makeText(getContext(), "点击了高亮区域", Toast.LENGTH_SHORT).show();
                    }
                }
                break;
            default:
     ...
     * @return
     */
    private boolean isTouchTargetView(int x, int y) {
        if (mTargetRect != null) {
            if (x >= mTargetRect.left && x <= mTargetRect.right && y >= mTargetRect.top && y <= mTargetRect.bottom) {
                return true;
            }
        }
        return false;
    ...
    public interface OnTargetViewClick {
        void onClick(View view);
    }

    public void setOnTargetViewClick(OnTargetViewClick onTargetViewClick) {
        this.onTargetViewClick = onTargetViewClick;
    }
}
